...
def process_video_frames_async(video_path, model, upload_dir, cam_height, file_name, original_name, gps_snap, interval_m, model_lock, is_360, location_str, callback, draw_grid=False, is_cancelled=None):
    ...
    if is_360 and fov_from_meta is None: fov_from_meta = 100.0
        ...
        if dist_accum >= interval_m or frame_idx == 0:
            ...
            try:
                defects, geo_feats, gen_files, footprints = process_single_image(
                    frame, model, frame_base_name, upload_dir, current_lat, current_lon, current_heading, cam_height, 
                    current_pitch, current_roll, current_base_pitch, current_base_roll, klns, fov_from_meta, model_lock, is_360, original_frame_name, draw_grid
                )
            except Exception as e:
