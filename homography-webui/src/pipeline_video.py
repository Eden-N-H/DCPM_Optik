...
def process_video_frames_async(video_path, model, upload_dir, file_name, original_name, location_str, options, model_lock, callback, is_cancelled=None, sam2_predictor=None, sam2_lock=None):
    ...
    while True:
        ...
        if dist_accum >= interval_m or frame_idx == 0:
            ...
            try:
                # Removed grid_baseline logic completely. 
                # This explicitly locks the BEV grid to the camera frame instead of the world map, 
                # ensuring the grid overlay acts as a fixed perspective HUD that is always perfectly horizontal.
