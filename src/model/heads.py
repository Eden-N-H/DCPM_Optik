...
class SegmentationHead(nn.Module):
    """Segmentation head: 2x 3x3 conv(128) + BN + ReLU → 1x1 conv(3) → upsample to 512x512."""
    ...
    def forward(self, x):
        ...
        x = self.classifier(x)  # [B, 3, 128, 128]
        x = F.interpolate(x, size=(512, 512), mode='bilinear', align_corners=False)
        return x  # [B, 3, 512, 512] logits
...
class SeverityHead(nn.Module):
    ...
    def forward(self, x):
        x = self.relu(self.bn1(self.conv1(x)))
        x = self.relu(self.bn2(self.conv2(x)))
        x = torch.sigmoid(self.regressor(x))  # [B, 1, 128, 128]
        x = F.interpolate(x, size=(512, 512), mode='bilinear', align_corners=False)
        return x  # [B, 1, 512, 512] in [0, 1]
