document.addEventListener("DOMContentLoaded", () => {
    ...
    const handleFiles = (files, isMulti, callback, nameElement) => {
        ...
        if (isMulti) {
            ...
            nameElement.textContent = `${files.length} file(s) queued`;
        } else {
            callback(files[0]);
    ...
    function loadWorkspace(resultsData, geojsonData) {
        // Ensure every result has a location key so grouping/filtering never
        // sees `undefined` (e.g. video frame results, which the backend
        // doesn't currently assign to a spatial cluster).
        fullResults = resultsData.map(r => ({
            ...r,
            location: r.location || "Video Frames"
        }));
