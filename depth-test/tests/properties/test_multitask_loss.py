...
# Property 18: Multi-task total loss computation
# =============================================================================
# For any non-negative loss components, total = 1.5×L_seg + 1.0×L_depth + 0.3×L_cam + 0.1×L_adv
# **Validates: Requirements 12.1**
...
def test_multitask_total_loss_weighted_summation(
    l_seg: float, l_depth: float, l_cam: float, l_adv: float
):
    """Property 18: For any non-negative loss components (L_seg, L_depth, L_cam,
    L_adv), the total loss SHALL equal
    1.5×L_seg + 1.0×L_depth + 0.3×L_cam + 0.1×L_adv.
...
def test_multitask_loss_uses_correct_weights_from_module(
    ...
    the specification: seg=1.5, depth=1.0, camera=0.3, adv=0.1.
    ...
    **Validates: Requirements 12.1**
    ...
    expected_total = 1.5 * l_seg + 1.0 * l_depth + 0.3 * l_cam + 0.1 * l_adv
...
def test_multitask_loss_segmentation_dominates(
    ...
    **Validates: Requirements 12.1**
    ...
    if l_seg > 0:
        ...
        depth_contribution = w_depth * l_seg
