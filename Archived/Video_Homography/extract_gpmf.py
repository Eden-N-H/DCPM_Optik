...
# ─────────────────────────────────────────────
# MP4 BOX WALKER (Kept from original)
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# GPMF DECODER (Enhanced with timing metadata)
# ─────────────────────────────────────────────
...
def _flatten_streams_with_timing(entries: list, streams: dict, target: Optional[str],
    ...
    # Pre-extract frequency clues if available inside this block
    # Total payload samples are distributed evenly across the chunk duration
    ...
    for entry in entries:
        ...
        if fourcc == "SCAL":
            ...
            # Calculate individual timestamps for each sample item in the payload array
            n_items = len(values)
            if n_items > 0:
                for idx, val in enumerate(values):
                    # Linearly spread samples within this chunk payload window
...
def _find_gpmf_samples_with_timing(mp4_bytes: bytes) -> list[tuple[int, int, float, float]]:
...
# ─────────────────────────────────────────────
# FRAME EXTRACTION & PROCESSING ENGINE
# ─────────────────────────────────────────────

def process_video_frames(mp4_path: Path, output_dir: Path, interpolators: dict, target_stream: str):
    """Iterates through video frames, synchronizes metrics, and provides homography placeholders."""
    cap = cv2.VideoCapture(str(mp4_path))
    if not cap.isOpened():
        print(f"Error: OpenCV could not open video file {mp4_path}")
        return

    fps = cap.get(cv2.CAP_PROP_FPS)
    total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
    print(f"Processing Video Stream: {total_frames} frames @ {fps:.2f} FPS")
    ...
    while True:
        ...
        # Calculate exact timestamp based on standard playback mapping
        frame_time = frame_idx / fps

        # Interp telemetry for this precise timestamp
        telemetry_snapshot = {}
        for sname, interp_func in interpolators.items():
            telemetry_snapshot[sname] = interp_func(frame_time).tolist()

        # ── HOMOGRAPHY PROCESSING BLOCK ──────────────────────────────────────
        # Placeholder: Generate your homography matrix H here based on telemetry.
        # e.g., mapping GYRO (angular velocity) or ACCL to planar translation matrices.
        H = np.eye(3, dtype=np.float32)

        # Example application: Warp image frame if H calculation is modified
        # warped_frame = cv2.warpPerspective(frame, H, (frame.shape[1], frame.shape[0]))
        # ─────────────────────────────────────────────────────────────────────

        # Save frame image asset
        frame_filename = frames_dir / f"frame_{frame_idx:05d}.jpg"
        cv2.imwrite(str(frame_filename), frame)

        log_entry = {
            "frame_index": frame_idx,
            "timestamp_sec": round(frame_time, 4),
    ...
    # Save mapping ledger configuration
    ledger_path = output_dir / "frame_telemetry_ledger.json"
    with open(ledger_path, "w") as f:
        json.dump(log_data, f, indent=2)
    print(f"\n[Success] Process completed. Visual assets and synchronized sync-ledger saved to:\n  {output_dir}")
...
def main():
    ap = argparse.ArgumentParser(description="Synchronize and extract GoPro video frames with precise stream telemetry metadata.")
    ap.add_argument("mp4", help="Filename inside Test_Videos/")
    ap.add_argument("--stream", "-s", default="GPS5", help="Primary telemetry stream target filter (e.g. GPS5, GYRO, ACCL)")
    args = ap.parse_args()
    ...
    if not mp4_path.exists():
        print(f"Error: Input video file not found at: {mp4_path}", file=sys.stderr)
        sys.exit(1)

    print(f"[1/3] Parsing GPMF binary streams from {mp4_path}...")
    try:
        streams = extract_streams_with_time(str(mp4_path))
        if not streams:
            print("Error: No valid telemetry streams found.")
            return

        print(f"Found streams: {list(streams.keys())}")

        print("[2/3] Constructing mathematical interpolation profiles...")
        interpolators = get_telemetry_interpolators(streams)

        print("[3/3] Commencing frame extraction pipeline...")
        process_video_frames(mp4_path, output_dir, interpolators, args.stream)

    except Exception as e:
        print(f"Processing Failure: {e}", file=sys.stderr)
        import traceback
        traceback.print_exc()
        sys.exit(1)
